...
PI <- predictInterval(merMod = flm3, newdata = fl11, level = 0.4, n.sims = 1000, stat = "median", type="linear.prediction", include.resid.var = TRUE)

PIwpops <- cbind(fl11$pop,PI)
...
PI <- predictInterval(merMod = flm3, newdata = fl11, level = 0.4, n.sims = 1000, stat = "median", type="linear.prediction", include.resid.var = TRUE)
PIwpops <- cbind(fl11$pop,fl11$ssp,PI)
colnames(PIwpops)[1] <- "pop"
colnames(PIwpops)[2] <- "ssp"

PIavg <- summaryBy(fit+ upr + lwr ~ pop + ssp, data = PIwpops, FUN = c(mean))
PIavg <- subset(PIavg, ssp=="w"|ssp=="t")
...
###PLOT OF CIs FOR 39 POPULATIONS
gg <-ggplot(aes(x=pop, y=fit.mean, ymin=lwr.mean, ymax=upr.mean), data=PIavg)
gg + geom_point() + geom_linerange() + labs(x="Populations", y="Prediction w/ 60% PI") + theme_bw()

###CALCULATION of AVERAGE CI
ciavg <- data.frame(PIavg$pop, upr = PIavg$upr.mean - PIavg$fit.mean, lwr = PIavg$fit.mean - PIavg$lwr.mean)
mean(ciavg$upr)
mean(ciavg$lwr)
